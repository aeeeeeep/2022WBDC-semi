...
def do_inference(batch, context, bindings, inputs, outputs, stream):
    ...
    inputs[1].host = np.ascontiguousarray(batch['frame_mask'], dtype=np.int32)
    inputs[2].host = np.ascontiguousarray(batch['text_input'], dtype=np.int32)
    inputs[3].host = np.ascontiguousarray(batch['text_mask'], dtype=np.int32)
